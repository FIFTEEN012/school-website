-- Add position_en column if it doesn't exist
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'personnel' AND column_name = 'position_en') THEN
        ALTER TABLE personnel ADD COLUMN position_en TEXT;
    END IF;
END $$;
