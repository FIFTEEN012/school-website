...
interface Image {
    url?: string; // Sometimes array of strings or objects? Supabase usually stores JSONB or array. Assuming array of objects or strings.
    image_url?: string;
    title?: string;
    description?: string;
}
...
interface Album {
  ...
  description?: string;
  category: string;
  images: any[]; // Adjust based on JSON structure
  created_at: string;
...
export default function Gallery({ albums }: GalleryProps) {
  ...
  // This depends on how `images` is stored. Assuming it's an array of objects in JSONB.
  const photos: Photo[] = albums.length > 0 ? albums.flatMap((album, albumIndex) => 
    (album.images || []).map((image: any, imageIndex: number) => ({
      ...
      image_url: image.url || image.image_url, // Handle different potential structures
  ...
  const categories = albums.length > 0 ? [
    ...
    ...Array.from(new Set(albums.map(a => a.category).filter(Boolean)))
      .map(cat => ({
        id: cat,
        label: cat, // You might want to map slug to label if needed
        icon: Camera
      }))
  ] : fallbackCategories;
